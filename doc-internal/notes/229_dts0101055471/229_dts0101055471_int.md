...
6.1.1 Adding new getPath() operations for needed model element to be called in
	getTitleTooltip() method is canceled, instead Cl_c.getPath() method is used.
	
	The method body is moved to HierarchyUtil in bp.core plugin to be accessible
	by all plugins.
...
Eclipse session started, it opened editor in lazy state, using the information
saved by Memento. As the icon was not stored, Eclipse get the editor icon using 
plugin.xml, which retain the editor icon instead of the element icon (as the 
getTitleImage() method did not invoked because of the lazy loading)

Therefore,  forcusGain() event is used for the three editor to force icon editor
