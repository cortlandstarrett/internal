...
public class Svx_c {
		...
		if (retSvxChannel != null) {
			...
			User_c.Logerror(
					"No Channel Configuration Was provided for Port :"
							...
							+"no SVX connection is established and value of 0.0 is returned. Check the port configuration table in SVX preferences",
					null);
		...
		if (!portTimedOut.get(port.getId())) {
			try {
				...
				if ( (nowTime + retSvxChannel.getSeconds())< retSvxChannel.getBIGendTime()) {
					...
					try {
						User_c.Loginfo("This port has exeeded the big end time value and 0.0 value will be returned");
			...
			User_c.Logerror(
					"This  Channel : "
							...
							+ port.getName()
							+ " has timed out and value of 0.0 will be returned. Try Increasing the Big End Time",
					null);
			Float result = (float) 0.0;
			return Gd_c.Real_to_instance(result);
		}
	...
	private static RequiredOperation_c retrieveRequiredOp(UUID id){
		RequiredOperation_c op = (RequiredOperation_c) Ooaofooa.getDefaultInstance()
	...
	private static ProvidedOperation_c retrieveProvicedOp(UUID id){
		ProvidedOperation_c op = (ProvidedOperation_c) Ooaofooa.getDefaultInstance()
	...
	public static void Setsvxvalue(UUID v_invocationId, UUID id, UUID value_id) {
		...
		if (LiteralReal_c.getOneV_LRLOnR801(retrievedValue)!=null)
		{
          valueToBeSent = Double.parseDouble(LiteralReal_c.getOneV_LRLOnR801(retrievedValue).getValue());
		...
		if (retSvxChannel != null) {
			...
			User_c.Logerror(
					"No Channel Configuration Was provided for Port :"
							+ retrievedPort.getName()
							+"no SVX connection is established. Check the port configuration table in SVX preferences",
					null);
